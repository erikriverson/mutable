...
#
#   Last saved
#    Time-stamp:      <2010-10-19 17:28:11 eriki>
#
...
muResponseTest.default <- function(x, strat, data, round.digits = 2, ...) {
  round(cor.test(x, strat, ...)$p.value, round.digits)
}

muResponseTest.factor <- function(x, strat, data, round.digits = 2, ...) {
  round(coef(summary(lm(strat ~ x, ...)))[2, 4], round.digits)
}
...
pvalHook <- function(ret) {
  ...
  makeNum <- function(x) c(x, testsuper = as.vector(unt[x$test]))
  lapply(ret, makeNum)
}
